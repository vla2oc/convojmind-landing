...
// S4. Текст — docs/COPY.md, раздел S4 (и «EN»).
...
export function Gap({ copy }: { copy: Copy["gap"] }) {
  return (
    <section>
      <div className="mx-auto max-w-5xl px-5 py-20 sm:py-24">
        ...
        <div className="mt-12">
          ...
          <div className="hidden gap-8 px-4 pb-3 text-sm text-muted sm:grid sm:grid-cols-[1fr_auto]">
            ...
            <span className="text-right">{copy.headRight}</span>
          </div>
            ...
            {copy.rows.map((row) => (
              <RevealItem as="li" hover key={row.accent} className={rowClass}>
                ...
                <div className="mt-3 flex flex-wrap gap-2 sm:mt-0 sm:justify-end">
                  <span className="sr-only">{copy.headRight}:</span>
                  {row.who.map((w) => (
                    <span
                      key={w}
        ...
        <Reveal className="mt-12 max-w-2xl">
          <p className="font-heading text-xl font-semibold leading-snug text-text sm:text-2xl">
            {copy.close.before}
            <span className="text-primary">{copy.close.accent}</span>
            {copy.close.after}
          </p>
          <p className="mt-4 leading-relaxed text-muted">{copy.quiet}</p>
        </Reveal>
      </div>
    </section>
  );
